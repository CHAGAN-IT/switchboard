...
**Domain:** Managed MCP (Model Context Protocol) hosting platform
...
**Decision guidance:** Use PyJWT alone if Switchboard is a *resource server only* (validates tokens issued by an external IdP like AWS Cognito or Auth0). Use Authlib if Switchboard also acts as its own authorization server. For v1, recommend PyJWT + external IdP (simpler, no key management burden).
...
uv add fastapi[standard] uvicorn[standard] httpx mcp fastmcp
...
# Dev dependencies
uv add --dev pytest pytest-asyncio respx httpx docker ruff mypy
```
...
**If running local development (Docker Compose):**
...
**If running in production (ECS Fargate):**
...
**If acting as OAuth2 resource server only (validates external tokens):**
...
**If acting as OAuth2 authorization server (issues tokens):**
...
---
*Stack research for: Managed MCP hosting platform (Switchboard)*
*Researched: 2026-04-14*
