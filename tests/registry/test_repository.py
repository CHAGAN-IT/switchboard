...
@pytest.mark.integration
async def test_create_server_duplicate_name(session, repo: ServerRepository) -> None:
    """create() raises DuplicateServerError for duplicate name."""
    await repo.create(session, name="dupe-server", container_image="img:v1")
    with pytest.raises(DuplicateServerError, match="dupe-server"):
        await repo.create(session, name="dupe-server", container_image="img:v2")


@pytest.mark.integration
async def test_get_by_name_existing(session, repo: ServerRepository) -> None:
    """get_by_name() returns the correct server."""
    created = await repo.create(session, name="find-me", container_image="img:latest")
...
@pytest.mark.integration
async def test_get_by_name_nonexistent(session, repo: ServerRepository) -> None:
...
@pytest.mark.integration
async def test_get_by_id_existing(session, repo: ServerRepository) -> None:
    """get_by_id() returns the correct server."""
    created = await repo.create(session, name="id-lookup", container_image="img:latest")
...
@pytest.mark.integration
async def test_get_by_id_nonexistent(session, repo: ServerRepository) -> None:
...
@pytest.mark.integration
async def test_list_all_returns_servers(session, repo: ServerRepository) -> None:
    """list_all() returns all servers in descending created_at order."""
    await repo.create(session, name="server-aaa", container_image="img:v1")
    await repo.create(session, name="server-bbb", container_image="img:v2")
...
@pytest.mark.integration
async def test_list_all_empty(session, repo: ServerRepository) -> None:
...
@pytest.mark.integration
async def test_update_status(session, repo: ServerRepository) -> None:
    ...
    updated = await repo.update_status(session, created.id, ServerStatus.running)
    assert updated is not None
    assert updated.status == ServerStatus.running


@pytest.mark.integration
async def test_update_status_nonexistent(session, repo: ServerRepository) -> None:
    """update_status() returns None for nonexistent server."""
    result = await repo.update_status(session, uuid.uuid4(), ServerStatus.running)
    assert result is None


@pytest.mark.integration
async def test_update_container_id_set(session, repo: ServerRepository) -> None:
    """update_container_id() sets the container_id field."""
    created = await repo.create(
        session, name="container-test", container_image="img:latest"
    )
    updated = await repo.update_container_id(session, created.id, "abc123def456")
    assert updated is not None
    assert updated.container_id == "abc123def456"


@pytest.mark.integration
async def test_update_container_id_clear(session, repo: ServerRepository) -> None:
...
@pytest.mark.integration
async def test_delete_existing(session, repo: ServerRepository) -> None:
    """delete() removes a server and returns True."""
    created = await repo.create(session, name="delete-me", container_image="img:latest")
...
@pytest.mark.integration
async def test_delete_nonexistent(session, repo: ServerRepository) -> None:
